...
// Set our breakpoint handler stub as the DBR (debug base register)
  ...
  // execution
  UBC_REG_CRR0 =
      (1 << UBC_CRR_RESERVED) | (1 << UBC_CRR_PCB) | (1 << UBC_CRR_BIE);
...
public:
  BreakpointDialog()
      : GUIDialog(Height55, AlignCenter, "Breakpoint Utility",
                  KeyboardStateABC),
        m_breakpointAddressLabel(GetLeftX() + 10, GetTopY() + 10,
                                 "Breakpoint address (hex)"),
        m_breakpointAddress(GetLeftX() + 10, GetTopY() + 30,
                            GetRightX() - GetLeftX() - 20, 8, true),
        m_setBreakpoint(GetLeftX() + 10, GetTopY() + 60, GetRightX() - 10,
                        GetTopY() + 90, "Set breakpoint",
                        BUTTON_SET_BREAKPOINT_EVENT_ID),
        m_removeBreakpoint(GetLeftX() + 10, GetTopY() + 95, GetRightX() - 10,
                           GetTopY() + 125, "Remove breakpoint",
                           BUTTON_REMOVE_BREAKPOINT_EVENT_ID),
        m_close(GetLeftX() + 10, GetTopY() + 130, GetRightX() - 10,
                GetTopY() + 160, "Close", BUTTON_CLOSE_EVENT_ID) {
  ...
  uint32_t GetBreakpointAddress() {
    ...
    if (!(address >= 0x80000000 && address <= 0x81500000) &&
  ...
  int OnEvent(struct GUIDialog_Wrapped *dialog,
              struct GUIDialog_OnEvent_Data *event) {
